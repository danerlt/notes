...
function npmRunPath(options = {}) {
    const { cwd = __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].cwd(), path: path_ = __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].env[pathKey()], execPath = __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].execPath } = options;
    let previous;
    const cwdString = cwd instanceof URL ? __WEBPACK_EXTERNAL_MODULE_node_url_e96de089__["default"].fileURLToPath(cwd) : cwd;
    let cwdPath = __WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].resolve(cwdString);
    const result = [];
    while(previous !== cwdPath){
        result.push(__WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].join(cwdPath, 'node_modules/.bin'));
        previous = cwdPath;
        cwdPath = __WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].resolve(cwdPath, '..');
    }
    result.push(__WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].resolve(cwdString, execPath, '..'));
    return [
        ...result,
        path_
    ].join(__WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].delimiter);
}
function npmRunPathEnv({ env = __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].env, ...options } = {}) {
    env = {
        ...env
    };
    const path = pathKey({
...
const makeError = ({ stdout, stderr, all, error, signal, exitCode, command, escapedCommand, timedOut, isCanceled, killed, parsed: { options: { timeout, cwd = __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].cwd() } } })=>{
...
const isSigterm = (signal)=>signal === __WEBPACK_EXTERNAL_MODULE_node_os_74b4b876__["default"].constants.signals.SIGTERM || 'string' == typeof signal && 'SIGTERM' === signal.toUpperCase();
...
const isExecaChildProcess = (target)=>target instanceof __WEBPACK_EXTERNAL_MODULE_node_child_process_27f17141__.ChildProcess && 'function' == typeof target.then;
const pipeToTarget = (spawned, streamName, target)=>{
    if ('string' == typeof target) {
        spawned[streamName].pipe((0, __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__.createWriteStream)(target));
        return spawned;
    }
...
const getInputSync = ({ input, inputFile })=>{
    ...
    return (0, __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__.readFileSync)(inputFile);
};
...
const getInput = ({ input, inputFile })=>{
    if ('string' != typeof inputFile) return input;
    validateInputOptions(input);
    return (0, __WEBPACK_EXTERNAL_MODULE_node_fs_5ea92f0c__.createReadStream)(inputFile);
};
...
const getBufferedData = async (stream, streamPromise)=>{
    ...
    await (0, __WEBPACK_EXTERNAL_MODULE_node_timers_promises_aedbf14c__.setTimeout)(0);
    stream.destroy();
    try {
...
const parseExpression = (expression)=>{
    ...
    if ('object' === typeOfExpression && null !== expression && !(expression instanceof __WEBPACK_EXTERNAL_MODULE_node_child_process_27f17141__.ChildProcess) && 'stdout' in expression) {
        const typeOfStdout = typeof expression.stdout;
        if ('string' === typeOfStdout) return expression.stdout;
        if (__WEBPACK_EXTERNAL_MODULE_node_buffer_fb286294__.Buffer.isBuffer(expression.stdout)) return expression.stdout.toString();
...
const verboseDefault = (0, __WEBPACK_EXTERNAL_MODULE_node_util_1b29d436__.debuglog)('execa').enabled;
...
const logCommand = (escapedCommand, { verbose })=>{
    if (!verbose) return;
    __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].stderr.write(`[${getTimestamp()}] ${escapedCommand}\n`);
};
...
const handleArguments = (file, args, options = {})=>{
    ...
    options = {
        ...
        extendEnv: true,
        preferLocal: false,
        localDir: options.cwd || __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].cwd(),
        execPath: __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].execPath,
        encoding: 'utf8',
        reject: true,
        cleanup: true,
        all: false,
    ...
    if ('win32' === __WEBPACK_EXTERNAL_MODULE_node_process_786449bf__["default"].platform && 'cmd' === __WEBPACK_EXTERNAL_MODULE_node_path_c5b9b54f__["default"].basename(file, '.exe')) args.unshift('/q');
    return {
        file,
        args,
        options,
...
const handleOutput = (options, value, error)=>{
    if ('string' != typeof value && !__WEBPACK_EXTERNAL_MODULE_node_buffer_fb286294__.Buffer.isBuffer(value)) return void 0 === error ? void 0 : '';
...
function execa(file, args, options) {
    ...
    let spawned;
    try {
        spawned = __WEBPACK_EXTERNAL_MODULE_node_child_process_27f17141__["default"].spawn(parsed.file, parsed.args, parsed.options);
    } catch (error) {
        const dummySpawned = new __WEBPACK_EXTERNAL_MODULE_node_child_process_27f17141__["default"].ChildProcess();
...
function execaSync(file, args, options) {
    ...
    let result;
    try {
        result = __WEBPACK_EXTERNAL_MODULE_node_child_process_27f17141__["default"].spawnSync(parsed.file, parsed.args, {
            ...parsed.options,
            input
        });
    } catch (error) {
